...
import DocObserverableRegisterService, { ArrayFieldObserverRegistration } from "../../register/DocObserverableRegisterService";
...
export default class ArrayFieldObserver extends ObserverBase{
    protected update(event: IObserverableEvent): void {
        if(event.type === 'ArrayObserved'){
            if(event.parentId){
                this.establishArrayObserver(event.parentId,event.propertyName,event.data as Y.Array<Y.Map<any>>)
            }
        }
    }
    private _yArrayFieldObserver = (
      ...
      private _establishArrayEntryObserver = (
        ...
        yArray.forEach(yMap => {
          this.notify({type: 'FieldObserved',propertyName:arrayEntryPropertyName, data:yMap});
        });
      };
}