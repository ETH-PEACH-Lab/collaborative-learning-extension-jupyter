...
import DocObserverableRegisterService, { FieldObserverRegistration } from "../../register/DocObserverableRegisterService";
...
export default class FieldObserver extends ObserverBase {
    protected update(event: IObserverableEvent): void {
        if (event.type === 'FieldObserved') {
            this._establichFieldObserver(event.propertyName, event.data as Y.Map<any>)
        }
    }
    ...
    private _establishFieldPropertyObserver(
        ...
        [...yMap.keys()].forEach(key => {
            ...
            if (
                ...
                this.notify({ type: 'ArrayObserved', parentId: yMap.get('id'), propertyName: propertyName, data: yMap.get(key) })
            }
            if (
