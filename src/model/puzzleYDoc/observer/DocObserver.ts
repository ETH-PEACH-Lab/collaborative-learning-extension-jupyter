...
export default class DocObserver implements IDocObserver {
  ...
  init(
    ...
    data.observeDeep(events =>
      this._yArrayFieldObserver(events, 'root', propertyName, arrayChange)
    );
  ...
  private _establishArrayObserver = (
    parentId: string,
    propertyName: string,
  ...
  private _establishFieldPropertyObserver(
    ...
    [...yMap.keys()].forEach(key => {
      ...
      if (
        DocObserverRegisterService.instance.cellFieldObservers.has(
          propertyName
        ) &&
        yMap.has(key)
      ) {
