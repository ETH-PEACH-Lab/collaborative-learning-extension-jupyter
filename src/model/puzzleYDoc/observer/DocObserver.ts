...
export default class DocObserver implements IDocObserver {
  ...
  constructor(emitChange: (change: PuzzleDocChange) => void) {
    this._observers.push(new ArrayFieldObserver(emitChange), new FieldObserver(emitChange))
    this._observers.forEach(observerA =>
      this._observers.forEach(observerB =>
        observerA.id !== observerB.id ?
          observerA.attach(observerB) : null
      )
    )
  }
  init(
    ...
    (this._observers[0] as ArrayFieldObserver).establishArrayObserver("", propertyName, data)
  }
